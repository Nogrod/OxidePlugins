...
{
    [Info("Economics", "Nogrod", "2.0.5", ResourceId = 717)]
    class Economics : RustPlugin
    {
        ...
        private void cmdTransfer(BasePlayer player, string command, string[] args)
        {
            ...
            var transferMoney = money*(1 - configData.Transfer_Fee);
            Deposit(targetPlayer.userID, transferMoney);
            PrintMessage(player, "Transfer_Success", targetPlayer.displayName, money);
            PrintMessage(targetPlayer, "Transfer_Success_To", player.displayName, transferMoney);
        }
        ...
        private void ccmdEco(ConsoleSystem.Arg arg)
        {
            ...
            switch (cmdArg)
            {
                ...
                case "setmoney":
                    ...
                    double money = Convert.ToDouble(arg.GetString(2, "0"));
                    if (money >= 0)
                    {
