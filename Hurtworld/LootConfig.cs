...
namespace Oxide.Plugins
{
    ...
    internal class LootConfig : HurtworldPlugin
    {
        ...
        private new bool LoadConfig()
        {
            try
            {
                Config.Settings = new JsonSerializerSettings { Converters = { new StringEnumConverter() } };
                if (!Config.Exists())
        ...
        private void OnServerInitialized()
        {
            ...
            {
                //sb.AppendLine(((EItemCode)item.Key) + " - " + LocalizationUtilities.TryTranslation(item.Value.GetNameKey()));
                sb.AppendLine(((EItemCode)item.Key) + " - " + item.Value.GetNameKey());
            }
            Puts(sb.ToString());*/
        }
        ...
        private void PrintLootNode(LootTreeNode lootNode, double parentChance, StringBuilder sb, int depth = 0, bool once = false)
        {
            ...
            if (itemGeneratorStatic != null)
            {
                ...
                if (itemGeneratorStatic.RandomVariance > 0) sb.Append($" (Rnd {itemGeneratorStatic.RandomVariance})");
